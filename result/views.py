...
def create_result(request):
  ...
  if request.method == 'POST':
        ...
        for student in students.split(','):
          ...
          for subject in subjects:
            check = Result.objects.filter(
                session=session, term=term, current_class=stu.current_class, subject=subject, student=stu).first()
            if not check:
              results.append(
                  Result(
                      session=session,
    ...
    if id_list:
      form = CreateResults(
          initial={"session": request.current_session, "term": request.current_term})
